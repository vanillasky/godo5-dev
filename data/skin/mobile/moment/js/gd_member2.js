...
var gd_member2 = (function ($, _, window, document, undefined) {
    ...
    messages = {
        ...
        email: __("정확한 이메일 주소를 입력해주시기 바랍니다."),
    ...
    function add_rules() {
        ...
        $.each(policy, function (idx, item) {
            ...
            if (item.require === 'y') {
                ...
                $markTarget.closest('td').prev('th').addClass('required');
            }
            if (item.minlen > 0) {
    ...
    function validation_submit(func) {
        ...
        var $marriDate = $('input[name="marriDate"]');
        if ($('.marridateinfo').has('dn') && $marriDate.length > 0) {
            $marriDate.rules("remove");
        }
